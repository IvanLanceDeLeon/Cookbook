...
function creatorCreate(first_name,last_name, cb){
...
function recipeCreate(name,creator,ingredients,type,instructions,cb){
    recipedetail = {
        name:name,
        creator:creator,
    ...
    if (type != false) recipedetail.type = type

    var recipe = new Recipe(recipedetail,cb);
    recipe.save(function (err) {
        if(err){
            cb(err,null)
            return
        }
        console.log("New RecipeL: " + recipe)
        recipes.push(recipe)
        cb(null, recipe)
    })
}
...
function createType(cb){
    async.series([
        function(callback){
            typeCreate('Soup',callback);
        },
        function(callback){
            typeCreate('Curry',callback);
        },
        function(callback){
            typeCreate('Rice Dish',callback);
        },
    ],
    cb)
}

function createCreator(cb) {
    async.series([
        function(callback) {
          creatorCreate('Patrick', 'Rothfuss', callback);
        },
        ],
        cb);
}
...
function createRecipe(cb){
    async.parallel([
        function(callback){
            recipeCreate('Ramen',creators[0],[ingredients[0], ], types[0], 'Boil Water', callback)
        },
        function(callback){
            recipeCreate('Fried Rice',creators[0],[ingredients[0],ingredients[1] ], types[0], 'Fry Rice', callback)
        },
    ],
    cb) 
}
